...
public:
    Image(const char* path, const Shape& shape)
        ...
        , gray_scale_{ -1.0 }
    {
        shape_.h = bmp_->getHeight();// set shape
    ...
    void resize(const Shape& shape)
    {
        ...
        // interpolation
        Bitmap* new_bmp = bilinear_interpolation(shape);
    ...
    {
        if (!bmp_->save(path)) {
            throw Error(string("Failed saving: ") + path);
        }
    }
...
private:
    ...
    Bitmap* bilinear_interpolation(const Shape& shape)
    {
...
public:
    ...
    {
        double min = fabs(tiles_[0]->get_gray_scale() - gray_scale);
        int m = 0;
        for (int i = 1; i < tiles_.size(); i++) {
            double diff = fabs(tiles_[i]->get_gray_scale() - gray_scale);
            if (diff < min - DBL_EPISION) {
                min = diff;
                m = i;
            }
        }
...
static void parse_shape(char* arg_cstr, Shape& output, Shape& cell)
{
    ...
    if (output.h % cell.h || output.w % cell.w) {
        throw Error("Bad size");//cannot do
    }
}
